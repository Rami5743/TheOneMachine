...
(async () => {
  ...
  const KEY='nand2tetris-lomda-v12', URL='http://localhost:8274/index.html';
  ...
  async function runCase(label, mutate) {
    const ws = JSON.parse(JSON.stringify(correct)); if (mutate) mutate(ws);
    ...
    const success = txt.includes("הבדיקה הצליחה");
    const fail = txt.includes("הבדיקה נכשלה");
    console.log(label, '-> success:', success, '| fail:', fail);
    await ctx.close();
  }
  await runCase("CORRECT", null);
  await runCase("NO-CONTROL", (ws)=>{ ws.wires = ws.wires.filter(w=>!(w.a==="task-card-1.inputInt2")); });
  await runCase("SWAP-OUTPUT (bits reversed)", (ws)=>{
    // reverse output legs -> wrong bit order -> should fail
    ws.wires = ws.wires.map(w=> w.a==="ff-1.out"?{a:"ff-1.out",b:"spout.leg3"}: w.a==="ff-4.out"?{a:"ff-4.out",b:"spout.leg0"}:w);
  });
  await b.close();
