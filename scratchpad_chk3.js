...
(async () => {
  ...
  const KEY='nand2tetris-lomda-v12', URL='http://localhost:8275/index.html';
  ...
  await runCase("SWAP bit0/bit3 output", (ws)=>{
    ws.wires = ws.wires.map(w=> w.b==="spout.leg0"?{a:w.a,b:"spout.leg3"}: w.b==="spout.leg3"?{a:w.a,b:"spout.leg0"}:w);
  });
  await runCase("CONTROL only 3 of 4 FFs", (ws)=>{ ws.wires = ws.wires.filter(w=>!(w.a==="task-card-1.inputInt2" && w.b==="ff-4.in2")); });
  await b.close();
