...
(async () => {
  ...
  const KEY='nand2tetris-lomda-v12', URL='http://localhost:8292/index.html';
  ...
  const okb = await p.$("[data-action='not-test-ok']");
  console.log('check result ok-button present:', !!okb);
  if (okb) { await okb.click({force:true}); await p.waitForTimeout(800); }
  const body = await p.evaluate(()=>document.body.innerText);
  console.log('walkthrough step1 shown:', body.includes("כרטיס זיכרון בנוי מפליפ-פלופים"));
  const acts = await p.$$eval('[data-action]', els=>[...new Set(els.map(e=>e.getAttribute('data-action')))].filter(a=>/solution/.test(a)));
  console.log('solution actions:', JSON.stringify(acts));
  for (let i=0;i<6;i++){ const n=await p.$("[data-action='solution-next']"); if(!n) break; await n.click({force:true}); await p.waitForTimeout(350); }
