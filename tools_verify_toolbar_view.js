...
const RAW = MODULE_SRC.split("isNandPresentationWorkspace }) {\n")[1].split("\n  return {")[0];
...
const NEW = new Function(stub + MODULE_SRC + "\n return { view: createToolbarView(DEPS), setCompleted, setPresentation };")();
...
[[], ["Not"], ["Not", "And", "Xor"], ["Bogus", "Or"], ["Not", "And", "Or", "Xor", "AND3way", "OR4way"]].forEach((completed, i) => {
  [false, true].forEach((presentation) => {
    OLD.setCompleted(completed); OLD.setPresentation(presentation);
    NEW.setCompleted(completed); NEW.setPresentation(presentation);
    cmp(`renderToolbar #${i} (completed=${JSON.stringify(completed)}, presentation=${presentation})`, OLD.renderToolbar(), NEW.view.renderToolbar());
  });
});
