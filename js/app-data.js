...
And ו־Not הם כרטיסים שמבצעים חישוב.`,
  ...
  const DEMO_TASKS = [
      ...
      solution: [
        {
          ...
          steps: [
            { code: "0 → D", bits: "1000000101010100", text: "זאת הפעולה שתחזור על עצמה. בחרנו פעולה שעושה בכל סיבוב בדיוק את אותו הדבר — כותבת 0 ל-D. אין שום חשיבות לאיזו פעולה בחרנו: המשימה רק דורשת שהמכונה תמשיך לבצע פקודות. אפשר היה אפילו לוותר על פקודה זו בכלל" },
            { code: "1 → A", bits: "0000000000011000", text: "פקודת קפיצה לא נושאת איתה לאן לקפוץ — המחשב תמיד קופץ למה שכתוב ברגיסטר A. לכן לפני הקפיצה נכניס ל-A את מספר הפקודה שאליה אנחנו רוצים לחזור, וזה המספר 1." },
        ...
        },
        {
          ...
          program: [
            { bits: "1000000101010100", label: "לולאה" },
            { bits: "0000000000011000", shows: "לולאה" },
            { bits: "1000000101010010" }
          ],
          steps: [
            { code: "לולאה", row: 0, field: "label",
              text: "השינוי הראשון: בעמודת התגיות שליד הפקודה הראשונה כתבנו לה שם — לולאה. הפקודה עצמה לא השתנתה באף ביט." },
            { code: "לולאה → A", row: 1, field: "number",
              text: "השינוי השני: במקום המספר 1 כתבנו כאן את שם התגית. הגמד מוצא באיזו שורה היא יושבת וממלא את אחד עשר הביטים בעצמו — אותם ביטים בדיוק שכתבנו קודם ביד." }
          ],
    ...
    },
    {
      ...
      hints: [
        { kind: "text", title: "רמז 1", text: "זאת לולאת הספירה מהמשימה הקודמת, רק שהפעם היא לא נמשכת לנצח: בכל סיבוב צריך לבדוק אם כבר הגענו למספר שב-In0." },
        { kind: "text", title: "רמז 2", text: "בסוף כל סיבוב תבדוק אם המספר שמוצג עדיין קטן מזה שב-In0. אם כן — תקפוץ אחורה ותוסיף לו עוד 1. אם לא — סיימת." },
        { kind: "text", title: "רמז 3", text: "איך בודקים אם מספר קטן ממספר אחר? מחסרים אותם. אם ההפרש שלילי אז הראשון קטן יותר, וזה בדיוק תנאי הקפיצה <0." },
        { kind: "text", title: "רמז 4", text: "כדי לחשב את ההפרש בין Out0 ל-In0 אתה צריך לגעת בשתי כתובות: קודם לקבוע את A על אחת ולהביא את הערך ל-D, ואז לקבוע את A על השנייה ולחשב מול D." },
        { kind: "text", title: "רמז 5", text: "גם המספר 0 צריך להופיע, אז תציג אותו לפני הלולאה. בתוך הלולאה קודם תוסיף 1 ואחר כך תבדוק — מכיוון ש-In0 הוא לפחות 1, ההוספה הראשונה תמיד נכונה." },
        { kind: "text", title: "רמז 6", text: "תן תגית לשורה שבה מתחילה הלולאה, והגמד ימלא את המספר בשבילך — וגם אם תוסיף שורות באמצע התוכנה זה עדיין יעבוד." },
        { kind: "text", title: "רמז 7", text: "כשהתנאי כבר לא מתקיים המחשב פשוט ממשיך לשורה הבאה, ואם היא ריקה — זהו. וזכור שהקפיצה דורסת את A, אז כל כתובת נקבעת מחדש בכל סיבוב." }
      ],
      solution: [
        {
          title: "פתרון",
          lead: "מציגים 0, ואז בכל סיבוב מוסיפים 1 ובודקים אם עוד לא הגענו. מכיוון שב-In0 יש לפחות 1, מותר להוסיף כבר בסיבוב הראשון — ולכן צריך כאן קפיצה אחת בלבד.",
          steps: [
            { code: "1024 → A", bits: "0100000000001000", text: "כתובת הפלט Out0. מתחילים ממנה." },
            { code: "0 → *A", bits: "1000000101011100", text: "מאפסים את Out0 — זה המספר הראשון שצריך להופיע." },
            { code: "לולאה:  1024 → A", bits: "0100000000001000", label: "לולאה", text: "כאן מתחילה הלולאה, ולכן יש לשורה תגית. שוב כתובת Out0 ל-A, כי הקפיצה שבסוף דרסה את A." },
            { code: "*A+1 → *A", bits: "1000011110111100", text: "מוסיפים 1. זה המספר הבא בספירה, והוא מוצג עכשיו ב-Out0." },
            { code: "*A → D", bits: "1000010000110100", text: "ולוקחים אותו ל-D. שים לב ש-A עדיין מכיל את 1024, אז אין צורך לקבוע אותו שוב." },
            { code: "1028 → A", bits: "0100000001001000", text: "ועכשיו כתובת הקלט In0 — המספר שאליו צריך להגיע." },
            { code: "D-*A → D", bits: "1000011100100100", text: "מחסרים: ב-D יש עכשיו Out0 פחות In0. אם הוא שלילי, עוד לא הגענו." },
            { code: "לולאה → A", bits: "0000000000111000", shows: "לולאה", text: "מכינים את היעד של הקפיצה אחורה: התגית לולאה, שורה 3." },
            { code: "D ; קפוץ אם <0", bits: "1000000011000001", text: "ואם עוד לא הגענו — חוזרים ומוסיפים עוד 1. כשההפרש כבר לא שלילי המחשב ממשיך לשורה הבאה, שהיא ריקה, ושם התוכנה נגמרת." }
          ],
          close: "תשע פקודות וקפיצה אחת. המספר האחרון שהופיע הוא בדיוק זה שב-In0, כי הבדיקה נעשית אחרי שהוא כבר הוצג."
        }
      ]
    },
    {
      ...
      hints: [
        { kind: "text", title: "רמז 1", text: "כפל הוא חיבור חוזר: להכפיל את In0 ב-In1 זה לחבר את In0 לעצמו In1 פעמים." },
        { kind: "text", title: "רמז 2", text: "אתה צריך מונה — מקום שסופר כמה חיבורים עוד נשארו. הפלט תפוס בתוצאה, אז שים את המונה באחת מכתובות הזיכרון הרגילות, למשל כתובת 0." },
        { kind: "text", title: "רמז 3", text: "התחל בלאפס את Out0 ולהעתיק את In1 למונה. אחר כך תיכנס ללולאה." },
        { kind: "text", title: "רמז 4", text: "בכל סיבוב: להוסיף את In0 ל-Out0, להוריד 1 מהמונה, ואז לבדוק אם המונה עדיין גדול מ-0 — ואם כן, לקפוץ אחורה. מכיוון של-In1 יש לפחות 1, החיבור הראשון תמיד נכון." },
        { kind: "text", title: "רמז 5", text: "אין תנאי קפיצה \"אם גדול מ-0\", אבל יש \"<0\". אז במקום לשאול אם המונה חיובי, תשאל אם מינוס המונה שלילי — זה בדיוק אותו דבר, וה-ALU יודע לחשב ‎-D." },
        { kind: "text", title: "רמז 6", text: "כדי להוסיף מספר לכתובת בזיכרון צריך 2 פקודות: קודם מביאים ל-D את המספר שרוצים להוסיף, ואז קובעים את A על היעד ומחשבים D+*A אל *A." },
        { kind: "text", title: "רמז 7", text: "כמו בכל הלולאות כאן — כל כתובת נקבעת מחדש בכל סיבוב, כי הקפיצה דרסה את A." }
      ],
      solution: [
        {
          title: "פתרון",
          lead: "מחברים את In0 לתוצאה שוב ושוב, In1 פעמים. אנחנו צריכים לדעת בכל שלב כמה פעמים עוד נשאר לחבר. נשים את זה בכתובת 0 של הזיכרון. זה יהיה ה\"מונה\" שלנו.",
          steps: [
            { code: "1024 → A", bits: "0100000000001000", text: "כתובת הפלט Out0." },
            { code: "0 → *A", bits: "1000000101011100", text: "מאפסים את התוצאה. אליה נחבר." },
            { code: "1029 → A", bits: "0100000001011000", text: "כתובת הקלט In1 — כמה פעמים לחבר." },
            { code: "*A → D", bits: "1000010000110100", text: "מביאים אותו ל-D." },
            { code: "0 → A", bits: "0000000000001000", text: "כתובת 0 בזיכרון הרגיל. שם יישב המונה." },
            { code: "D → *A", bits: "1000000011001100", text: "וכותבים אליה את In1. זה המונה." },
            { code: "לולאה:  1028 → A", bits: "0100000001001000", label: "לולאה", text: "כאן מתחילה הלולאה, ולכן יש לשורה תגית. מתחילים מכתובת הקלט In0 — המספר שמחברים." },
            { code: "*A → D", bits: "1000010000110100", text: "מביאים אותו ל-D." },
            { code: "1024 → A", bits: "0100000000001000", text: "וכתובת הפלט Out0." },
            { code: "D+*A → *A", bits: "1000010100001100", text: "מחברים: התוצאה שהייתה ועוד In0, בחזרה ל-Out0." },
            { code: "0 → A", bits: "0000000000001000", text: "חוזרים לכתובת של המונה." },
            { code: "*A-1 → *A", bits: "1000010100111100", text: "ומורידים ממנו 1 — חיבור אחד פחות נשאר." },
            { code: "*A → D", bits: "1000010000110100", text: "ולוקחים את מה שנשאר ל-D. A עדיין 0, אז אין צורך לקבוע אותו שוב." },
            { code: "לולאה → A", bits: "0000000001111000", shows: "לולאה", text: "מכינים את היעד של הקפיצה אחורה: התגית לולאה, שורה 7." },
            { code: "-D ; קפוץ אם <0", bits: "1000001111000001", text: "וזאת הבדיקה: אין תנאי \"אם גדול מ-0\", אז מחשבים מינוס המונה ושואלים אם הוא שלילי — זה קורה בדיוק כשהמונה חיובי. כשהמונה מגיע ל-0 המחשב ממשיך לשורה הבאה, שהיא ריקה, ושם התוכנה נגמרת." }
          ],
          close: "חמש עשרה פקודות וקפיצה אחת. שים לב שהמכונה מבצעת כאן In1 סיבובים — להכפיל ב-1000 ייקח לה פי אלף יותר זמן מלהכפיל ב-1."
        }
      ]
    },
    {
      ...
      hints: [
        { kind: "text", title: "רמז 1", text: "אם כפל הוא חיבור חוזר, חילוק הוא חיסור חוזר: כמה פעמים אפשר להוריד את In1 מ-In0 לפני שנגמר." },
        { kind: "text", title: "רמז 2", text: "אתה צריך מקום לשמור את מה שנשאר. הפלט תפוס במנה, אז שים את מה שנשאר באחת מכתובות הזיכרון הרגילות, למשל כתובת 0." },
        { kind: "text", title: "רמז 3", text: "התחל בלאפס את Out0 ולהעתיק את In0 לכתובת שבה תשמור את מה שנשאר." },
        { kind: "text", title: "רמז 4", text: "אל תבדוק לפני החיסור. פשוט תחסר ותספור, ותמשיך כל עוד מה שנשאר עדיין אינו שלילי — כך צריך קפיצה אחת בלבד." },
        { kind: "text", title: "רמז 5", text: "כשיוצאים מהלולאה ספרת חיסור אחד יותר מדי — זה שגלש מתחת ל-0. לכן בסוף פשוט תוריד 1 מהמנה." },
        { kind: "text", title: "רמז 6", text: "אין תנאי קפיצה \"אם אינו שלילי\", אבל יש \"≤0\". אז במקום לשאול אם מה שנשאר אינו שלילי, תשאל אם מינוס מה שנשאר אינו חיובי — זה בדיוק אותו דבר, וה-ALU יודע לחשב ‎-D." },
        { kind: "text", title: "רמז 7", text: "כמו תמיד — כל כתובת נקבעת מחדש בכל סיבוב, כי הקפיצה דרסה את A." }
      ],
      solution: [
        {
          title: "פתרון",
          lead: "מורידים את In1 ממה שנשאר וסופרים, בלי לבדוק קודם — וממשיכים כל עוד מה שנשאר לא ירד מתחת ל-0. בסוף מורידים 1 מהמנה, כי החיסור האחרון היה אחד יותר מדי.",
          steps: [
            { code: "1024 → A", bits: "0100000000001000", text: "כתובת הפלט Out0." },
            { code: "0 → *A", bits: "1000000101011100", text: "מאפסים את המנה. אותה נספור." },
            { code: "1028 → A", bits: "0100000001001000", text: "כתובת הקלט In0 — המספר שמחלקים." },
            { code: "*A → D", bits: "1000010000110100", text: "מביאים אותו ל-D." },
            { code: "0 → A", bits: "0000000000001000", text: "כתובת 0 בזיכרון הרגיל." },
            { code: "D → *A", bits: "1000000011001100", text: "ושם נשמור את מה שנשאר. בהתחלה זה כל In0." },
            { code: "לולאה:  1029 → A", bits: "0100000001011000", label: "לולאה", text: "כאן מתחילה הלולאה, ולכן יש לשורה תגית. מתחילים מכתובת הקלט In1 — המספר שמחסרים." },
            { code: "*A → D", bits: "1000010000110100", text: "מביאים אותו ל-D." },
            { code: "0 → A", bits: "0000000000001000", text: "וחוזרים לכתובת של מה שנשאר." },
            { code: "*A-D → *A", bits: "1000011110001100", text: "מחסרים בלי לבדוק קודם: מה שנשאר, פחות In1, בחזרה לאותה כתובת." },
            { code: "1024 → A", bits: "0100000000001000", text: "כתובת המנה." },
            { code: "*A+1 → *A", bits: "1000011110111100", text: "ומוסיפים לה 1 — חיסרנו עוד פעם אחת." },
            { code: "0 → A", bits: "0000000000001000", text: "חוזרים לכתובת של מה שנשאר." },
            { code: "*A → D", bits: "1000010000110100", text: "ולוקחים אותו ל-D כדי לבדוק אותו." },
            { code: "לולאה → A", bits: "0000000001111000", shows: "לולאה", text: "מכינים את היעד של הקפיצה אחורה: התגית לולאה, שורה 7." },
            { code: "-D ; קפוץ אם ≤0", bits: "1000001111000011", text: "וזאת הבדיקה: אין תנאי \"אם אינו שלילי\", אז מחשבים מינוס מה שנשאר ושואלים אם הוא אינו חיובי — זה קורה בדיוק כשמה שנשאר עדיין 0 או יותר." },
            { code: "1024 → A", bits: "0100000000001000", text: "יצאנו מהלולאה. כתובת המנה." },
            { code: "*A-1 → *A", bits: "1000010100111100", text: "ומורידים ממנה 1: החיסור האחרון גלש מתחת ל-0, אז ספרנו אותו לחינם. אחריו השורה ריקה, ושם התוכנה נגמרת." }
          ],
          close: "שמונה עשרה פקודות וקפיצה אחת. אין כאן שום בדיקה לפני הפעולה — מחסרים, סופרים, וברגע שגלשנו מתקנים את הספירה באחד."
        }
      ]
    }
  ];
  ...
  const CASES_TASKS = [
    {
      ...
      // which is what "טעינת תוכנה" is for.
      hints: [
        { kind: "text", title: "רמז 1", text: "אל תסתכל על זה כעל משימה אחת. אלו חמש תוכנות קטנות ועוד החלטה אחת בראש: לקרוא את In2 ולקפוץ לחלק המתאים. שתיים מחמש התוכנות האלה כבר כתבת — הכפל והחילוק של הפרק הקודם — ואם שמרת אותן, אתה יכול לטעון אותן ולהעתיק אותן לכאן." },
        { kind: "text", title: "רמז 2", text: "תן תגית לשורה הראשונה של כל אחד מחמשת החלקים: חיבור, חיסור, כפל, חילוק, ריבוע. ככה ההחלטה בראש התוכנה יכולה לקפוץ לכל חלק בשם שלו, ואתה לא צריך לדעת מראש באיזו שורה כל חלק מתחיל — הגמד ימלא את המספרים במקומך. תגית של לולאה תקרא בשם אחר, כדי שלא תתבלבל בין השתיים: \"תחילת לולאת\" ואחריה שם הפעולה." },
        { kind: "text", title: "רמז 3", text: "ההחלטה עצמה היא שרשרת בדיקות. פקודת קפיצה לא יודעת להשוות שני מספרים — היא רק בודקת אם התוצאה של ה-ALU היא 0 או שלילית. אז כדי לשאול \"האם In2 שווה ל-2\" תחשב In2 פחות 2, ותקפוץ אם התוצאה היא 0." },
        { kind: "text", title: "רמז 4", text: "כדי לחסוך פקודות, תביא את In2 ל-D פעם אחת בלבד בראש התוכנה, ואז תבדוק אותו שוב ושוב מול 0, 1, 2, 3 — בכל פעם תקבע את A על המספר שאתה משווה אליו וקפוץ אם ההפרש הוא 0. אם אף אחת מארבע הבדיקות לא קפצה, מה שנשאר הוא הפעולה החמישית, ואפשר פשוט להמשיך אליה." },
        { kind: "text", title: "רמז 5", text: "חיבור וחיסור לא צריכים לולאה בכלל: תביא את In0 ל-D, תקבע את A על הכתובת של In1, ותחשב D+*A (או D-*A). רק שים לב לאן התוצאה הולכת — ברגע הזה ב-A יושבת כתובת של קלט, ולשם אי אפשר לכתוב. תשמור את התוצאה ב-D, ורק אחר כך תקבע את A על Out0 ותכתוב אותה לשם. זו כל התוכנה — שש פקודות." },
        { kind: "text", title: "רמז 6", text: "בכפל ובחילוק אל תתחיל מאפס. פתח את \"טעינת תוכנה\", טען את הפתרון ששמרת מהמשימה המתאימה, סמן בחלונית את הפקודות עם התגיות שלהן והעתק אותן לתוך החלק שלהן כאן. שים לב שלא יהיו שתי תגיות באותו שם בדף — התוכנות שהעתקת יושבות עכשיו זו לצד זו." },
        { kind: "text", title: "רמז 7", text: "ריבוע הוא כפל של In0 בעצמו. אותו קוד בדיוק כמו הכפל, רק שבמקום להביא את In1 בתור המונה — כמה פעמים לחבר — תביא את In0." },
        { kind: "text", title: "רמז 8", text: "לכל חלק צריך סוף, אחרת הוא ימשיך לזלוג לתוך החלק הבא. תן תגית לשורה ריקה שאחרי כל חמשת החלקים — קרא לה \"סוף\" — ובסוף כל חלק תקפוץ אליה. שם התוכנה נעצרת, כי אין שם פקודות." }
      ],
      solution: [
      {
        title: "פתרון: ההחלטה",
        lead: "לפני שהמכונה מחשבת משהו היא צריכה לדעת מה ביקשו ממנה. זה החלק היחיד שבאמת חדש כאן — כל השאר הן תוכנות שכבר כתבת.",
        steps: [
          { code: "1030 → A", bits: "0100000001101000", text: "הכתובת של In2 — הפורט השלישי. שם יושב מספר הפעולה שביקשו מאיתנו." },
          { code: "*A → D", bits: "1000010000110100", text: "ומביאים אותו ל-D. פעם אחת בלבד: מכאן והלאה כל הבדיקות נעשות על D, בלי לחזור לזיכרון." },
          { code: "חיבור → A", bits: "0000010001001000", shows: "חיבור", text: "לפני כל קפיצה צריך לומר לאן. כאן זו השורה שבה מתחיל החיבור — ובמקום מספר כתבנו את שם התגית שלה, והגמד ימלא את הביטים." },
          { code: "D ; קפוץ אם =0", bits: "1000000011000010", text: "הבדיקה הראשונה. ה-ALU מחשב את D עצמו, והקפיצה קורית אם התוצאה היא 0 — כלומר אם מספר הפעולה הוא 0. אם לא, ממשיכים לשורה הבאה." },
          { code: "D-1 → D", bits: "1000000111000100", text: "וכאן הרעיון שחוסך את כל השאר: במקום להביא שוב את In2 ולחסר ממנו 1, פשוט מורידים 1 מ-D. עכשיו \"מספר הפעולה הוא 1\" הפך לשאלה \"האם D הוא 0\" — אותה בדיקה בדיוק." },
          { code: "חיסור → A", bits: "0000010011001000", shows: "חיסור", text: "היעד הבא: החיסור." },
          { code: "D ; קפוץ אם =0", bits: "1000000011000010", text: "ואותה בדיקה, שעכשיו שואלת על 1." },
          { code: "D-1 → D", bits: "1000000111000100", text: "עוד אחד למטה." },
          { code: "כפל → A", bits: "0000000111111000", shows: "כפל", text: "היעד הבא: הכפל." },
          { code: "D ; קפוץ אם =0", bits: "1000000011000010", text: "ועכשיו הבדיקה שואלת על 2." },
          { code: "D-1 → D", bits: "1000000111000100", text: "ועוד אחד." },
          { code: "חילוק → A", bits: "0000001100001000", shows: "חילוק", text: "היעד הבא: החילוק." },
          { code: "D ; קפוץ אם =0", bits: "1000000011000010", text: "ועל 3. ואם גם כאן לא קפצנו — נשארה בדיוק אפשרות אחת, ריבוע, ואין צורך לבדוק אותה בכלל: היא פשוט השורה הבאה." }
        ],
        close: "שלוש עשרה פקודות, ואף אחת מהן לא מחשבת כלום. כל מה שהן עושות הוא להחליט לאן ללכת."
      },
      {
        title: "פתרון: חמשת החלקים",
        lead: "ומכאן זה כבר מוכר. חמשת החלקים יושבים אחד אחרי השני, לכל אחד תגית בשורה הראשונה שלו — התגיות שההחלטה קופצת אליהן. כל חלק עומד בפני עצמו, ושניים מהם הם התוכנות ששמרת בפרק הקודם, מועתקות פנימה כמו שהן.",
        program: [
          { bits: "0100000001101000" },
          { bits: "1000010000110100" },
          { bits: "0000010001001000", shows: "חיבור" },
          { bits: "1000000011000010" },
          { bits: "1000000111000100" },
          { bits: "0000010011001000", shows: "חיסור" },
          { bits: "1000000011000010" },
          { bits: "1000000111000100" },
          { bits: "0000000111111000", shows: "כפל" },
          { bits: "1000000011000010" },
          { bits: "1000000111000100" },
          { bits: "0000001100001000", shows: "חילוק" },
          { bits: "1000000011000010" },
          { bits: "0100000000001000", label: "ריבוע" },
          { bits: "1000000101011100" },
          { bits: "0100000001001000" },
          { bits: "1000010000110100" },
          { bits: "0000000000001000" },
          { bits: "1000000011001100" },
          { bits: "0100000001001000", label: "תחילת לולאת ריבוע" },
          { bits: "1000010000110100" },
          { bits: "0100000000001000" },
          { bits: "1000010100001100" },
          { bits: "0000000000001000" },
          { bits: "1000010100111100" },
          { bits: "1000010000110100" },
          { bits: "0000000101001000" },
          { bits: "1000001111000001" },
          { bits: "0000010100101000", shows: "סוף" },
          { bits: "1000000101010010" },
          { bits: "0100000000001000", label: "כפל" },
          { bits: "1000000101011100" },
          { bits: "0100000001011000" },
          { bits: "1000010000110100" },
          { bits: "0000000000001000" },
          { bits: "1000000011001100" },
          { bits: "0100000001001000", label: "תחילת לולאת כפל" },
          { bits: "1000010000110100" },
          { bits: "0100000000001000" },
          { bits: "1000010100001100" },
          { bits: "0000000000001000" },
          { bits: "1000010100111100" },
          { bits: "1000010000110100" },
          { bits: "0000001001011000" },
          { bits: "1000001111000001" },
          { bits: "0000010100101000", shows: "סוף" },
          { bits: "1000000101010010" },
          { bits: "0100000000001000", label: "חילוק" },
          { bits: "1000000101011100" },
          { bits: "0100000001001000" },
          { bits: "1000010000110100" },
          { bits: "0000000000001000" },
          { bits: "1000000011001100" },
          { bits: "0100000001011000", label: "תחילת לולאת חילוק" },
          { bits: "1000010000110100" },
          { bits: "0000000000001000" },
          { bits: "1000011110001100" },
          { bits: "0100000000001000" },
          { bits: "1000011110111100" },
          { bits: "0000000000001000" },
          { bits: "1000010000110100" },
          { bits: "0000001101101000" },
          { bits: "1000001111000011" },
          { bits: "0100000000001000" },
          { bits: "1000010100111100" },
          { bits: "0000010100101000", shows: "סוף" },
          { bits: "1000000101010010" },
          { bits: "0100000001001000", label: "חיבור" },
          { bits: "1000010000110100" },
          { bits: "0100000001011000" },
          { bits: "1000010100000100" },
          { bits: "0100000000001000" },
          { bits: "1000000011001100" },
          { bits: "0000010100101000", shows: "סוף" },
          { bits: "1000000101010010" },
          { bits: "0100000001001000", label: "חיסור" },
          { bits: "1000010000110100" },
          { bits: "0100000001011000" },
          { bits: "1000011100100100" },
          { bits: "0100000000001000" },
          { bits: "1000000011001100" },
          { bits: "", label: "סוף" }
        ],
        steps: [
          { code: "1024 → A", row: 13, text: "כאן מתחיל הריבוע, והדבר הראשון הוא לאפס את Out0: התוצאה נבנית בו מאפס, ומה שהיה בו קודם לא שייך לנו." },
          { code: "1028 → A", row: 15, text: "In0 — וכאן הוא בתפקיד המונה: כמה פעמים צריך לחבר." },
          { code: "D → *A", row: 18, text: "המונה נשמר בכתובת 0 של הזיכרון, בדיוק כמו בכפל של הפרק הקודם." },
          { code: "1028 → A", row: 19, text: "תחילת הלולאה, ולכן יש לשורה תגית — ותגית של לולאה נקראת כאן תמיד \"תחילת לולאת\" ואחריה שם הפעולה, כך שרואים מיד לאן כל קפיצה חוזרת. מכאן והלאה זו בדיוק לולאת הכפל שכתבת בפרק הקודם: מחברים את In0 ל-Out0, מורידים 1 מהמונה, ובודקים אם נשאר עוד." },
          { code: "-D ; קפוץ אם <0", row: 27, text: "היציאה מהלולאה. אין תנאי קפיצה \"אם גדול מ-0\", אז שואלים על מינוס המונה אם הוא קטן מ-0 — וזה אותו דבר." },
          { code: "0 ; קפוץ אם =0", row: 29, text: "וכשהלולאה נגמרה, קופצים לסוף. בלי הקפיצה הזאת הריבוע היה ממשיך לזלוג לתוך הכפל שמתחתיו." },
          { code: "1024 → A", row: 30, text: "והכפל הוא אותו חלק בדיוק, פקודה אחרי פקודה." },
          { code: "1029 → A", row: 32, text: "עם הבדל אחד: המונה שלו נלקח מ-In1 — כמה פעמים לחבר את In0 — ולא מ-In0 עצמו." },
          { code: "1024 → A", row: 47, text: "החילוק מועתק מהפרק הקודם בלי שינוי: מורידים את In1 מ-In0 שוב ושוב, וסופרים כמה פעמים הספקנו." },
          { code: "1029 → A", row: 53, text: "הלולאה שלו." },
          { code: "*A-1 → *A", row: 64, text: "והתיקון שבסופה: הלולאה סופרת גם את הפעם שבה כבר עברנו את האפס, אז מורידים אחד בחזרה." },
          { code: "1028 → A", row: 67, text: "החיבור הוא הקצר מכולם: In0 ל-D, ואז D+*A כשב-A יושבת הכתובת של In1. התוצאה נשמרת ב-D ולא נכתבת ישר לפלט — ב-A יושבת עכשיו כתובת של קלט, ולשם אי אפשר לכתוב — ולכן שתי הפקודות האחרונות קובעות את A על Out0 וכותבות אותה לשם. שש פקודות, בלי לולאה." },
          { code: "1028 → A", row: 75, text: "והחיסור זהה לו לגמרי חוץ מפקודה אחת: D-*A במקום D+*A." },
          { code: "סוף", row: 81, text: "והשורה הזאת ריקה — יש לה רק תגית. כל חלק שסיים את שלו קופץ לכאן, וכאן המכונה נעצרת, כי אין כאן פקודה." }
        ],
        close: "שמונים ואחת פקודות, ואף אחת מהן אינה חדשה. מה שחדש הוא ששלוש עשרה פקודות בראש התוכנה בוחרות איזו מהן תרוץ."
      },
      {
        title: "פתרון קצר יותר",
        lead: "עכשיו תסתכל על הריבוע ועל הכפל זה לצד זה. חוץ מפקודה אחת — מאיפה בא המונה, מ-In0 או מ-In1 — הם אותן שבע עשרה פקודות פעמיים. קוד שכתוב פעמיים הוא לא רק ארוך פי שניים: כשתמצא בו טעות תצטרך לתקן אותה בשני מקומות, וביום שתשכח, שתי הפעולות יפסיקו לעשות את אותו הדבר. אז נשאיר עותק אחד, ונשלח את הריבוע אליו.",
        program: [
          { bits: "0100000001101000" },
          { bits: "1000010000110100" },
          { bits: "0000001110111000", shows: "חיבור" },
          { bits: "1000000011000010" },
          { bits: "1000000111000100" },
          { bits: "0000010000111000", shows: "חיסור" },
          { bits: "1000000011000010" },
          { bits: "1000000111000100" },
          { bits: "0000000101101000", shows: "כפל" },
          { bits: "1000000011000010" },
          { bits: "1000000111000100" },
          { bits: "0000001001111000", shows: "חילוק" },
          { bits: "1000000011000010" },
          { bits: "0100000000001000", label: "ריבוע" },
          { bits: "1000000101011100" },
          { bits: "0100000001001000" },
          ...
          { bits: "0000000111001000" },
          { bits: "1000001111000001" },
          { bits: "0000010010011000", shows: "סוף" },
          { bits: "1000000101010010" },
          { bits: "0100000000001000", label: "חילוק" },
          { bits: "1000000101011100" },
          { bits: "0100000001001000" },
        ...
        steps: [
          { code: "1024 → A", row: 13, text: "החלק של הריבוע התקצר משבע עשרה פקודות לשמונה. ההתחלה שלו לא השתנתה בכלל: מאפסים את Out0, ולוקחים את In0 בתור המונה." },
          { code: "תחילת לולאת כפל → A", row: 19, text: "וכאן הכול שונה. במקום לולאה משלו, הריבוע מצביע על הלולאה של הכפל." },
          { code: "0 ; קפוץ אם =0", row: 20, text: "וקופץ אליה. שתי הפקודות האלה החליפו אחת עשרה." },
          { code: "1024 → A", row: 21, text: "הכפל עצמו לא השתנה באף ביט: הוא מאפס את Out0 ולוקח את In1 בתור המונה, כמו קודם." },
          { code: "1028 → A", row: 27, text: "וזאת הלולאה היחידה שנשארה — זו שהריבוע קופץ אליה. היא לא צריכה לדעת מי קרא לה: שני החלקים כבר הכינו את המונה שלהם לפניה, והיא רק מחברת את In0 שוב ושוב עד שהוא נגמר." }
        ],
        close: "תשע פקודות פחות — שבעים ושתיים במקום שמונים ואחת — ולולאה אחת במקום שתיים. המחיר הוא שתי פקודות הקפיצה שהריבוע משלם בכל הרצה, ובבדיקה במכונה זה ארבעה צעדים בסך הכול. זו העסקה הרגילה בתכנות, ולרוב היא משתלמת."
      }
    ]
    }
  ];
  ...
  // The three tasks that have an ANSWER and then stop. Each case is In0, In1 and
  // what Out0 should hold when the machine has settled; Out0 starts at something
  // other than 0, since a reset does not wipe the memory.
  const PROGRAM_COUNT_TO_TEST = {
    kind: "answer",
    climb: true,
    title: "בדיקת התוכנה",
    cycles: 3000,
    ...
    passTitle: "הבדיקה הצליחה",
    passText: "התוכנה ספרה מ-0 עד המספר שב-In0, הראתה את כל המספרים בדרך, ונעצרה עליו.",
    failTitle: "הבדיקה נכשלה",
    emptyText: "עדיין לא כתבת אף פקודה.",
    ranOffText: "אחרי {steps} פקודות המכונה הגיעה לשורה {line}, שלא כתבת בה כלום.",
    incompleteText: "פקודה {line} לא הושלמה.",
    noStopText: "כשב-In0 היה {in0} התוכנה לא נעצרה — Out0 המשיך להשתנות עד סוף הבדיקה.",
    wrongText: "אחרי {prev} הופיע ב-Out0 {got} במקום {want}.",
    wrongAnswerText: "כשב-In0 היה {in0}, התוכנה נעצרה על {got} במקום על {want}.",
    captions: {
      load: "התוכנה נטענת לזיכרון התוכנה",
      connect: "הריסט מתנתק",
      run: "המכונה רצה",
      check: "בודקים על מה נעצר Out0",
      reset: ""
    }
  };

  const PROGRAM_MULTIPLY_TEST = {
    kind: "answer",
    title: "בדיקת התוכנה",
    cycles: 4000,
    startOut: 500,
    ...
    passTitle: "הבדיקה הצליחה",
    passText: "בכל המקרים שנבדקו התוכנה כתבה ב-Out0 את המכפלה של In0 ב-In1 ונעצרה.",
    failTitle: "הבדיקה נכשלה",
    emptyText: "עדיין לא כתבת אף פקודה.",
    ranOffText: "אחרי {steps} פקודות המכונה הגיעה לשורה {line}, שלא כתבת בה כלום.",
    incompleteText: "פקודה {line} לא הושלמה.",
  ...
  const PROGRAM_CALC_TEST = {
    ...
    startOut: 500,
    cases: [
      { in: [7, 3, 0], out: [10] },  { in: [12, 9, 0], out: [21] },
      { in: [7, 3, 1], out: [4] },   { in: [12, 9, 1], out: [3] },
      { in: [7, 3, 2], out: [21] },  { in: [12, 9, 2], out: [108] },
      { in: [7, 3, 3], out: [2] },   { in: [100, 10, 3], out: [10] },
      { in: [7, 3, 4], out: [49] },  { in: [12, 9, 4], out: [144] }
    ],
    passTitle: "הבדיקה הצליחה",
  ...
  const PROGRAM_DIVIDE_TEST = {
    ...
    passTitle: "הבדיקה הצליחה",
    passText: "בכל המקרים שנבדקו התוכנה כתבה ב-Out0 את המנה השלמה של In0 ב-In1 ונעצרה.",
    failTitle: "הבדיקה נכשלה",
    emptyText: "עדיין לא כתבת אף פקודה.",
    ranOffText: "אחרי {steps} פקודות המכונה הגיעה לשורה {line}, שלא כתבת בה כלום.",
    incompleteText: "פקודה {line} לא הושלמה.",
