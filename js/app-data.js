  ...
  const DEMO_TASKS = [
    ...
    },
    {
      ...
      hints: [
        ...
        { kind: "text", title: "רמז 2", text: "למחשב אין תנאי קפיצה \"אם שונה מ-0\". יש לו =0, <0 ו-≤0. אז במקום לקפוץ אחורה כל עוד לא סיימנו, נקפוץ קדימה — אל הסוף — ברגע שסיימנו." },
        { kind: "text", title: "רמז 3", text: "איך בודקים אם שני מספרים שווים? מחסרים אותם. אם ההפרש הוא 0 הם שווים, וזה בדיוק תנאי הקפיצה =0." },
        ...
        { kind: "text", title: "רמז 5", text: "כדי לקפוץ לסוף אתה צריך לדעת באיזו שורה הוא. תן לשורה הזאת תגית והגמד ימצא את המספר בשבילך — וגם אם תוסיף שורות באמצע התוכנה זה עדיין יעבוד." },
        { kind: "text", title: "רמז 6", text: "\"לעצור\" זה שתי פקודות: אחת שמכניסה ל-A את מספר השורה של עצמה, ואחת שקופצת. מכאן המכונה רצה במקום ולא משנה יותר כלום." },
        { kind: "text", title: "רמז 7", text: "אל תשכח שהקפיצה דורסת את A. בכל חזרה לתחילת הלולאה צריך לקבוע מחדש כל כתובת שאתה ניגש אליה." }
      ],
      solution: [
        {
          title: "פתרון",
          lead: "לולאת הספירה של המשימה הקודמת, ובתוכה בדיקה: אם המספר שמוצג כבר שווה לזה שב-In0 — קופצים לסוף.",
          steps: [
            { code: "1024 → A", bits: "0100000000001000", text: "כתובת הפלט Out0. מתחילים ממנה." },
            { code: "0 → *A", bits: "1000000101011100", text: "מאפסים את Out0 — משם מתחילה הספירה." },
            ...
            { code: "*A → D", bits: "1000010000110100", text: "לוקחים ל-D את המספר שמוצג עכשיו ב-Out0." },
            { code: "1028 → A", bits: "0100000001001000", text: "ועכשיו כתובת הקלט In0 — המספר שאליו צריך להגיע." },
            { code: "D-*A → D", bits: "1000011100100100", text: "מחסרים: ב-D יש עכשיו Out0 פחות In0. אם זה 0 — הגענו." },
            { code: "סוף → A", bits: "0000000011011000", shows: "סוף", text: "מכינים את היעד של הקפיצה קדימה: שורת הסיום. הגמד ממלא את המספר לפי התגית." },
            { code: "D ; קפוץ אם =0", bits: "1000000011000010", text: "וזאת הבדיקה: ה-ALU מוציא את D, ואם הוא 0 — קופצים לסוף. אחרת ממשיכים לשורה הבאה." },
            { code: "1024 → A", bits: "0100000000001000", text: "עוד לא הגענו, אז סופרים הלאה: כתובת Out0." },
            { code: "*A+1 → *A", bits: "1000011110111100", text: "ומוסיפים לו 1." },
            { code: "לולאה → A", bits: "0000000000111000", shows: "לולאה", text: "היעד של הקפיצה אחורה: התגית לולאה, שורה 3." },
            { code: "0 ; קפוץ אם =0", bits: "1000000101010010", text: "וחוזרים לתחילת הלולאה." },
            { code: "סוף:  סוף → A", bits: "0000000011011000", label: "סוף", shows: "סוף", text: "כאן התוכנה נעצרת. התגית סוף מצביעה על השורה הזאת עצמה." },
            { code: "0 ; קפוץ אם =0", bits: "1000000101010010", text: "וקופצים אליה שוב ושוב. המכונה רצה במקום, ו-Out0 נשאר על המספר שהגיע אליו." }
          ],
          close: "ארבע עשרה פקודות. שורות 3 עד 12 הן לולאת הספירה של המשימה הקודמת, עם בדיקה שנוספה לה באמצע ושתי שורות עצירה בסוף."
        }
      ]
    },
    {
      ...
      solution: [
        {
          ...
          steps: [
            ...
            { code: "0 ; קפוץ אם =0", bits: "1000000101010010", text: "וחוזרים לתחילת הלולאה." },
            { code: "סוף:  סוף → A", bits: "0000000100111000", label: "סוף", shows: "סוף", text: "כאן התוכנה נעצרת." },
            { code: "0 ; קפוץ אם =0", bits: "1000000101010010", text: "המכונה רצה במקום, והתוצאה נשארת ב-Out0." }
          ],
          close: "עשרים פקודות: שש להתארגנות, שתים עשרה בלולאה ושתיים לעצירה. שים לב שהמכונה מבצעת כאן In1 סיבובים — להכפיל ב-1000 ייקח לה פי אלף יותר זמן מלהכפיל ב-1."
        }
      ]
    },
    {
      ...
      solution: [
        {
          ...
          steps: [
            ...
            { code: "0 ; קפוץ אם =0", bits: "1000000101010010", text: "וחוזרים לתחילת הלולאה." },
            { code: "סוף:  סוף → A", bits: "0000000100111000", label: "סוף", shows: "סוף", text: "כאן התוכנה נעצרת." },
            { code: "0 ; קפוץ אם =0", bits: "1000000101010010", text: "המכונה רצה במקום, והמנה נשארת ב-Out0." }
          ],
          close: "עשרים פקודות, ובדיוק אותו מבנה כמו הכפל — רק שבמקום לחבר, מחסרים, ובמקום לספור עד שנגמר המונה, סופרים עד שנגמר מה שנשאר."
        }
      ]
    }
  ];
